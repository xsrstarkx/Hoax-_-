...
var { ttdown } = require(__path + '/lib/tiktok.js')
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
     ...
     if (!url) return res.json(loghandler.noturl)
      ...
      TikTokScraper.getVideoMeta(url, options).then(vid => { console.log(vid),
      ttdown(url, options).then(resul => { console.log(resul)
             res.json({
                 status: true,
                 creator: `${creator}`,
		 results: resul.result,
		 videoNoWm: vid
             })
            })
	   })
